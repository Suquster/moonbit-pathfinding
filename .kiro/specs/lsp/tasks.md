...
- [ ] 1. 协议层头部帧处理（`src/lsp_binding/framing.mbt`，旁路新增）
  - [-] 1.1 实现 `Content-Length` 头部帧编解码与流式读取器
...
- [ ] 2. 协议层批量请求（`src/lsp_binding/batch.mbt`，旁路新增）
  - [~] 2.1 实现批量解码、逐条分发与编码
...
- [ ] 3. 协议层取消与 id 关联（`src/lsp_binding/cancel.mbt` + `correlate.mbt`，旁路新增）
  - [~] 3.1 实现取消登记表与取消响应
...
- [~] 4. 检查点 —— 确保协议层（framing/batch/cancel/correlate）全部测试通过
  - 在 `wasm-gc` / `js` / `native` 三后端运行 `src/lsp_binding` 至此为止的单元与属性测试；运行 native 前先 `export LIBRARY_PATH=/usr/lib64:/usr/lib`。
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. 生命周期状态机与能力协商（`src/lsp_server/lifecycle.mbt` + `capabilities_ext.mbt`，旁路新增）
  - [~] 5.1 实现生命周期纯状态机与会话句柄
...
- [ ] 6. 位置编码与坐标换算（`src/lsp_server/position_encoding.mbt`，旁路新增）
  - [~] 6.1 实现位置编码协商与 Position↔offset 换算
...
- [ ] 7. 增量文档同步（`src/lsp_server/sync.mbt`，旁路新增）
  - [~] 7.1 实现文档同步类别与变更应用
...
- [~] 8. 检查点 —— 确保会话基础设施（lifecycle/capabilities/position_encoding/sync）全部测试通过
  - 在三后端运行 `src/lsp_server` 至此为止的单元与属性测试；运行 native 前先 `export LIBRARY_PATH=/usr/lib64:/usr/lib`。
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. 引用查找、文档/工作区符号与高亮（`src/lsp_server/references.mbt`，旁路新增）
  - [~] 9.1 在 `analyze` 投影之上实现符号/引用/高亮能力
...
- [ ] 10. 重命名（`src/lsp_server/rename.mbt`，旁路新增）
  - [~] 10.1 实现 WorkspaceEdit 与重命名
...
- [ ] 11. 格式化与代码动作（`src/lsp_server/format.mbt`，旁路新增）
  - [~] 11.1 实现格式化与代码动作
...
- [ ] 12. 签名帮助、语义着色与折叠（`src/lsp_server/semantic.mbt`，旁路新增）
  - [~] 12.1 实现 signatureHelp/semanticTokens/foldingRange
...
- [ ] 13. 诊断模型 push + pull（`src/lsp_server/diagnostics.mbt`，旁路新增）
  - [~] 13.1 实现 pull 诊断与诊断 JSON 编码
  ...
  - [~] 13.2 诊断单元测试（严重码映射/push 通知构造/pull 响应）
...
- [~] 14. 检查点 —— 确保扩展能力（references/rename/format/semantic/diagnostics）全部测试通过
  - 在三后端运行 `src/lsp_server` 至此为止的全部单元与属性测试；运行 native 前先 `export LIBRARY_PATH=/usr/lib64:/usr/lib`。
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 15. 既有兼容回归与非法输入错误条件（向后兼容硬约束）
  - [~] 15.1 编写属性测试：既有分析不变
...
- [ ] 16. 端到端会话 demo（`src/lsp_server/demo.mbt`，旁路新增）
  - [~] 16.1 实现贯穿全流程的会话脚本
...
- [ ] 18. 集成、文档与双包发布推进
  ...
  - [~] 18.5 推进 `lsp_binding` 独立 SemVer 与 CHANGELOG
  ...
  - [~] 18.6 推进 `lsp_server` SemVer 与 CHANGELOG
