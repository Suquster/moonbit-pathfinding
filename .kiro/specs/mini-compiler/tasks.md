...
- [ ] 1. 旁路类型层、提升桥与测试生成器基础（`ml_types.mbt`）
  - [ ] 1.1 实现 MiniML 旁路类型层与 `of_minilet` 提升桥
  ...
  - [ ]* 1.2 新增属性测试生成器集 `ml_gen_test.mbt`
    ...
    - _Requirements: 12.6_
  - [ ]* 1.3 向后兼容属性测试 `prop_compat_test.mbt`
  ...
  - [ ]* 1.4 MiniLet 黄金样例单元测试 `compat_golden_test.mbt`
...
- [ ] 2. MiniML 词法、语法与打印（`ml_lexer.mbt` / `ml_parser.mbt` / `ml_printer.mbt`）
  - [ ] 2.1 实现 MiniML 词法分析 `ml_lexer.mbt`
  ...
  - [ ] 2.2 实现 MiniML 递归下降语法分析 `ml_parser.mbt`
  ...
  - [ ]* 2.5 词法 / 语法 / 打印单元测试 `ml_frontend_unit_test.mbt`
...
- [ ] 4. 合一、替换与 occurs-check（`unify.mbt`）
  - [ ] 4.1 实现合一与替换核心 `unify.mbt`
  ...
  - [ ]* 4.2 合一正确性属性测试 `prop_unify_correct_test.mbt`
  ...
  - [ ]* 4.3 occurs-check 属性测试 `prop_occurs_check_test.mbt`
  ...
  - [ ]* 4.4 替换幂等属性测试 `prop_subst_idem_test.mbt`
...
- [ ] 5. Hindley-Milner 类型推断 Algorithm W（`infer.mbt`）
  ...
  - [ ]* 5.2 主类型存在性属性测试 `prop_principal_type_test.mbt`
  ...
  - [ ]* 5.3 推断幂等属性测试 `prop_infer_idem_test.mbt`
  ...
  - [ ]* 5.4 推断单元测试 `infer_unit_test.mbt`
...
- [ ] 7. 树遍历解释器与作用域检查（`ml_eval.mbt`）
  - [ ] 7.1 实现解释器与作用域检查 `ml_eval.mbt`
  ...
  - [ ]* 7.2 提升保持语义属性测试 `prop_lift_test.mbt`
  ...
  - [ ]* 7.3 求值确定性属性测试 `prop_eval_determinism_test.mbt`
  ...
  - [ ]* 7.4 良类型不卡住属性测试 `prop_type_safety_test.mbt`
  ...
  - [ ]* 7.5 作用域检查属性测试 `prop_scope_test.mbt`
  ...
  - [ ]* 7.6 解释器与作用域单元测试 `eval_ml_unit_test.mbt`
...
- [ ] 8. 字节码编译后端与栈式虚拟机（`bytecode.mbt` / `vm.mbt` / `text_backend.mbt`）
  - [ ] 8.1 实现字节码模型与编译器 `bytecode.mbt`
  ...
  - [ ] 8.2 实现栈式虚拟机 `vm.mbt`
    ...
    - _Requirements: 6.3_
  - [ ] 8.3 实现额外文本后端 `text_backend.mbt`（可选特性 R6.4）
    - `emit_wat(te)`、`emit_js(te)` 渲染为 wasm 文本 / js 源串，仅供展示
    - _Requirements: 6.4_
  - [ ]* 8.4 编译—执行等价属性测试 `prop_compile_vm_test.mbt`
  ...
  - [ ]* 8.5 字节码 / VM 单元测试 `bytecode_vm_unit_test.mbt`
...
- [ ] 10. AST 优化（`optimize.mbt`）
  - [ ] 10.1 实现优化管线 `optimize.mbt`
  ...
  - [ ]* 10.2 优化保持可推断性属性测试 `prop_opt_typeable_test.mbt`
  ...
  - [ ]* 10.3 优化保持语义属性测试 `prop_opt_semantics_test.mbt`
  ...
  - [ ]* 10.4 捕获避免替换属性测试 `prop_beta_capture_test.mbt`
...
- [ ] 11. 统一流水线集成（`pipeline.mbt`）
  ...
  - [ ]* 11.2 端到端两路一致属性测试 `prop_pipeline_test.mbt`
...
- [ ] 12. 旗舰端到端示例（`demo.mbt`）
  ...
  - [ ]* 12.2 端到端示例单元测试 `demo_unit_test.mbt`
...
- [ ] 14. 性能基准（`benches/mini_compiler_bench/`）
  - [ ] 14.1 新增基准包 `benches/mini_compiler_bench/`
...
- [ ] 15. 可执行文档扩充（`README.mbt.md`）
  - [ ] 15.1 扩充 `src/mini_compiler/README.mbt.md`
...
- [ ] 16. 发布推进（`CHANGELOG.md` / `release.mbt`）
  - [ ] 16.1 推进 SemVer 与发布门禁
