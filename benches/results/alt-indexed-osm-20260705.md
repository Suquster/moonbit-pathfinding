...
## 追加（同日）：radix heap 替换大权 4 叉编码堆

大权（>1024）heap 路径（dijkstra_indexed_ctx / astar_indexed_ctx /
astar_biheap_ctx）的 4 叉编码堆换成 65 桶基数堆（AMOT 1990，单调
队列，push O(1)、pop 摊还 O(64)，免比较下滤）。一致启发式保证键
单调不减（等 f 键仅节点位倒序，最小性不受影响）。OSM 复测两轮：

| 指标 | EncodedHeap | RadixHeap |
|---|---|---|
| 厦门 单向 Dijkstra | 912.6 µs | 877.5–881.0 µs |
| 厦门 双向 Dijkstra | 626.4 µs | 643.5–644.8 µs（持平） |
| 北京 单向 Dijkstra | 9903.4 µs | 7934.9–7996.0 µs（**1.24×**） |
| 北京 双向 Dijkstra | 7353.3 µs | 6139.2–6152.4 µs（**1.20×**） |
| 北京 ALT 双向 A* | 1113.8 µs | 945.1–947.5 µs（**1.18×**，vs 双向 6.48–6.51×） |

三后端 2145/2145 全绿；heap 权域差分 PBT 覆盖单调性正确性。
