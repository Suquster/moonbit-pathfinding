...
- [x] G-B 深度闭包：性能/形式化前沿（2026-07-08 收官）
  ...
  - [x] G-B4++++++++++++++++++++++++++++ INFRA 第二十九批覆盖收口
        （lsp_server rope_sync：joinLeft 双旋/递归直返、joinRight
        回溯左旋修复；infra_text rope：多叶 UTF-16→UTF-8 右子树
  ...
  - [x] G-B4+++++++++++++++++++++++++++++++ INFRA 第三十二批覆盖收口
        （serialization proto_grammar：EOF 处标识符截断、repeated
        后期望字段类型缺失、嵌套 message 错误向上传播；advanced ch：
        退化捷径拒绝、层级不变量违例与缺失降级；三后端 2963→2967
        全绿）。2026-07-09，
  ...
  - [x] G-B4++++++++++++++++++++++++++++++++++++ INFRA 第三十七批覆盖收口
        （serialization proto_service：顶层声明非标识符拒绝、
  ...
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++ INFRA 第四十批覆盖收口
        （build_tool rule_grammar：游离 TAB recipe 行忽略、空变量
  ...
  - [x] G-B4++++++++++++++++++++++++++++++++++++++++++ INFRA 第四十三批覆盖收口
        （logging sampling：边界率量化钳制；lsp_server
        jsonrpc_frame：尾随空白裁剪、空数值段、同长键失配、
  ...
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++++++++++++++ INFRA 第五十二批
        虚假闭包完备化续五：TOML 字符串规范收口（toml.io v1.0.0
        §String —— 多行 """/''' 结束定界符紧邻的 1–2 个引号内容
        （引号跑判定，连续 6+ 引号拒绝）、空多行字符串、未转义控制
        字符（U+0000–U+0008/U+000B/U+000C/U+000E–U+001F/U+007F 与
        单行裸 CR）一律拒绝而多行放行换行/制表；移除已过时的
  ...
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第七十八批 余支收口 + OSM 空白判定拔高：
        osm_is_trim_space 纳入 `\n`（仅含换行的内容此前误走解析
        失败降级而非「内容仅含空白」诊断，属真实实现缺口）；DES
        未知来源/未知目标/已崩溃目标消息余臂、DesResult 全前缀
        截断拒绝；畸形矩阵/CSR 防御裁剪与 to_successors 投影；
        inflate_preset 畸形拒绝与 zlib CM 校验；alt 空分支/many1
        非推进防护；document_highlights 引用/空白/无定义余臂；
        Dijkstra/AStar 陈旧堆条目惰性跳过。src/ 未覆盖 351→336
        行，3234→3250 三后端全绿。2026-07-12。
  - [x] G-B4++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第七十九批 余支覆盖收口（codegen_infra + bench +
        regex）：解释器步数上限停机；evaluate 未知标签/坏版本号/
        未知算符左值回退；GVN 菱形双臂含字面量操作数的 PRE 上提
        合并；George 高度数邻居拒绝；基准报告 JSON 整型/浮点类型
        不符与 \u 截断拒绝；PikeVM 最左最长后到更左候选替换。
        另判定 alt/ch 惰性删除余臂、rope 旋转兜底、zstd 编码器
        护栏为不可达防御，保留不造假态。src/ 未覆盖 336→327 行，
        3250→3256 三后端全绿。2026-07-12。
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第八十批 余支覆盖收口（rope + codegen_infra）：大绳
        Concat 结构上的 offset_of_line/line_col_at 与朴素扫描差分；
        孤立高代理项前缀按 3 字节计长；CopyProp 无版本左值不参与
        传播、φ 实参替换为复制源版本；select VarRef 显式加载模板；
  ...
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第八十二批 余支覆盖收口（infra_compress + mini_compiler +
        parser_combinator）：lz4 字面量后 offset 截断拒绝；zstd 字典帧
        校验和篡改/截断双拒绝（xxhash64 低 32 位）；let rec 非函数体
        退化为普通绑定；source_line_at 行号越界空串；choice 零分支
        软失败。另判定 gzip FEXTRA 截断（多帧头 18 字节前置校验保
        证）、zlib FLG 补偿（cmf·256+flg 恒被 31 整除）、字典 FSE
  ...
  - [x] G-B4++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第八十五批 余支覆盖收口（lsp_server）：validate_jsonrpc
        响应非法 id 类型（布尔）拒绝；decode_frame 同长度异名头字段
        逐字符不等臂（Content-Weight）；analyze CRLF 归一化（\r 跳
        过）；folding_ranges 前导空白内容行聚块；apply_changes 负行
        号拒绝。另判定 dsl 键名前导空白修剪臂（行起点恒非空白）为
        不可达防御，保留不造假态。src/ 未覆盖 288→283 行，3299→3304
        三后端全绿。2026-07-12。
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第八十六批 余支覆盖收口（infra_compress）：zstd 含序列
        块尾部剩余字面量超 max_out 预算拒绝（编码器构造真实序列块，
        预算差 1 字节命中尾部检查）；纯字面量块超预算拒绝对照。另判
        定 huf_build_table cur!=size（total+rest 恒等于 size）、
        huf_decode_stream nb==0（合法建表填满全表）、seq_table 默认/
        ncount 后建表失败、of/ml/ll 码域越界（表 max_symbol 已限
        定）、字典 FSE 建表失败、多帧 budget<0 与帧尾 out>max_out
        （逐帧预算护栏保证）为不可达防御，保留不造假态。src/ 未覆盖
        283→282 行，3304→3306 三后端全绿。2026-07-12。
  - [x] G-B4++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第八十七批 余支覆盖收口（logging/infra_time）：
        parse_logfmt 未知 level 标签、字段整数溢出（Int64）、取值缺
        失（键后行尾）、取值后粘连字符四重拒绝；DateTime Show 即
        ISO-8601 渲染；days_in_month 越界月份为 0；parse_posix_tz
  ...
  - [x] G-B4+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INFRA 第九十批 余支覆盖收口（infra_diff/infra_cli/infra_config）：
        semver 预发布字母段前缀比较（短者优先级低）；validate_rules
        VBool 畸形值上报；suggest_option 等距候选字典序决胜；
        ini_parse 空键行拒绝。src/ 未覆盖 265→264 行，3324→3328
        三后端全绿。2026-07-12。
