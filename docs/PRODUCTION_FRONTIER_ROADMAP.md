...
>
> **重要立场（用户明确要求）**：本纲领**不被 spec/附录里「玩具/教学级」「非目标」
> 「实现边界」等措辞封顶**。凡是真实前沿成熟实现具备、而本库声明「不做」的能力
> （真·并发运行时、真·wasm 产物、真·跨语言互通、真·导出器…），都视为**要深化、
> 拔高、加难度去攻克的目标**，而非可回避的边界。对标的是**当下各领域工业界最前沿、
> 最成熟的真实落地实现**，不是研究/教学示意。
...
**生产化任务**（**突破** spec 的「玩具/不生成原生产物」非目标）：
- [ ] **T1.1 真·WebAssembly 产物**：把字节码 lower 到**合法 wasm 模块**（`.wasm` 二进制 /
      wat），可被真实 wasm 运行时（wasmtime/浏览器）加载执行，结果与解释器一致。
- [ ] **T1.2 真·JS 产物**：emit 可被 Node 直接执行的 JS，端到端结果一致。
- [ ] **T1.3 类型推断深化**：let-多态泛化/实例化、代数数据类型穷尽性检查、行多态。
- [ ] **T1.4 字节码优化 + 诊断**：常量传播/DCE/跳转线程化（等价 PBT）；类型错误带期望/实际/源位置/修复建议（对标 Rust/Elm 诊断）。
...
**生产化任务**（**突破** spec 的「不做真实导出/不接 async」非目标）：
- [ ] **T7.1 真·OTLP 导出器**：按 OpenTelemetry OTLP（protobuf/JSON）线缆格式序列化 span，
      产出可被真实 collector 接收的字节（复用方向九的 protobuf 编码）。
...
**生产化任务**（**突破** spec 的「不接真并发/async」非目标）：
- [ ] **T10.1 真·异步运行时**：若 `moonbitlang/async` 可登记进 `moon.mod.json`，
      把同步调度替换为真实异步消息循环（对外契约不变）；否则实现协作式调度器逼近。
- [ ] **T10.2 监督树语义**：one-for-one / one-for-all / rest-for-one 重启策略 +
      重启强度/周期上限，带确定性测试。
