...
| 平台轴 | 单后端 ⊂ 三后端一致(wasm-gc/native/js) ⊂ 浏览器交付(Pages playground) ⊂ 包注册表交付(mooncakes v0.0.5) ⊂ 四后端一致(+纯 wasm 线性内存) ⊂ WASI 运行时交付(wasmtime) ⊂ wasm 组件模型交付(wasm-tools componentize) ⊂ WIT 显式接口导出(类型化 world + canonical ABI 提升) | 全八级落地（2026-07-09：wasm 后端 2683 测试入 CI 矩阵；WASI 交付门禁 scripts/wasi_gate.sh；组件模型门禁 scripts/component_gate.sh；WIT 接口门禁 scripts/wit_gate.sh — wit/playground.wit 类型化 world、接口反解一致、wasmtime --invoke 逐值校验）；下一级=多组件组合 |
