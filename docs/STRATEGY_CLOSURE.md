...
| 平台轴 | 单后端 ⊂ 三后端一致(wasm-gc/native/js) ⊂ 浏览器交付(Pages playground) ⊂ 包注册表交付(mooncakes v0.0.5) ⊂ 四后端一致(+纯 wasm 线性内存) | 全五级落地（2026-07-09：wasm 后端 2600 测试 + 12 份可执行文档全绿并入 CI 矩阵）；下一级=wasm 组件模型/WASI 交付 |
