...
test "README · spawn 派生 actor 并经 send 投递消息" {
  ...
  counter.send(2)
  counter.send(3)
  @test.assert_eq(sched.pending(counter), 3)
  // run_until_idle 串行处理全部消息：1 + 2 + 3 = 6
  sched.run_until_idle()
  @test.assert_eq(sched.state_of(counter), Some(6))
  @test.assert_eq(sched.pending(counter), 0)
...
test "README · send 保持 FIFO 顺序且 step 单步串行处理" {
  ...
  @test.assert_eq(sched.state_of(log), Some("a"))
  @test.assert_eq(sched.pending(log), 2)
  // 第二步处理 "b"
  assert_true(sched.step())
  @test.assert_eq(sched.state_of(log), Some("ab"))
  // 第三步处理 "c"；FIFO 顺序使结果为 "abc"（R10.4）
  assert_true(sched.step())
  @test.assert_eq(sched.state_of(log), Some("abc"))
  // 邮箱已空：step 不再有进展（空邮箱挂起，R10.5）
  @test.assert_eq(sched.step(), false)
}
```
...
test "README · stop 在处理完当前消息后停止 actor" {
  ...
  acc.send(2)
  sched.run_until_idle()
  @test.assert_eq(sched.state_of(acc), Some(3))
  ...
  match sched.status_of(acc) {
    Some(Stopped) => ()
    other => fail("期望 Stopped，实际 \{@debug.to_repr(other)}")
  }
...
test "README · 未捕获错误终止 actor 并通知 supervisor（错误隔离）" {
  ...
  match sched.status_of(worker) {
    Some(Failed(reason)) => @test.assert_eq(reason, "除零错误")
    other => fail("期望 Failed，实际 \{@debug.to_repr(other)}")
  }
  assert_false(sched.is_running(worker))
  // supervisor 恰好收到一次通知（R10.6）
  @test.assert_eq(notes.length(), 1)
  @test.assert_eq(notes[0], "actor #1: 除零错误")
  // 错误隔离：healthy 不受影响，照常完成 7 + 8 = 15 且仍在运行
  @test.assert_eq(sched.state_of(healthy), Some(15))
...
test "README · OneForOne 监督仅重启失败的子 actor" {
  ...
  @test.assert_eq(sys.restart_count(worker_a), 1)
  @test.assert_eq(sys.state_of(worker_a), Some(3))
  assert_true(sys.is_running(worker_a))
  // 错误隔离：worker_b 零重启、状态完好、仍在运行
  @test.assert_eq(sys.restart_count(worker_b), 0)
  @test.assert_eq(sys.state_of(worker_b), Some(10))
...
test "README · 三种监督策略的处置范围与重启强度上界" {
  // 5 个子，失败者下标为 2
  // OneForOne：仅失败者 {2}
  @test.assert_eq(affected_children(SupervisionStrategy::OneForOne, 5, 2), [2])
  // OneForAll：全体 {0,1,2,3,4}
  @test.assert_eq(affected_children(SupervisionStrategy::OneForAll, 5, 2), [
    0, 1, 2, 3, 4,
  ])
  // RestForOne：失败者及其后 {2,3,4}
  @test.assert_eq(affected_children(SupervisionStrategy::RestForOne, 5, 2), [
    2, 3, 4,
  ])
...
test "README · become 切换处理行为，unbecome 恢复且弹空为空操作" {
  ...
  a.send(4) // 累加 → 6 + 4 = 10
  sys.run_until_idle()
  @test.assert_eq(sys.state_of(a), Some(10))
...
test "README · stash 暂存消息，unstash_all 按原序回流处理" {
  ...
  sys.run_until_idle()
  // 暂存的 1 + 2 + 3 按原序被累加 = 6
  @test.assert_eq(sys.state_of(a), Some(6))
  assert_true(sys.is_running(a))
}
```
...
test "README · 三种路由策略：RoundRobin / Broadcast / ConsistentHash" {
  ...
  @test.assert_eq(rr_sys.pending(rr_workers[0]), 2)
  @test.assert_eq(rr_sys.pending(rr_workers[1]), 2)
  @test.assert_eq(rr_sys.pending(rr_workers[2]), 2)
  ...
  bc.route(2)
  bc.route(3)
  @test.assert_eq(bc_sys.pending(bc_workers[0]), 3)
  @test.assert_eq(bc_sys.pending(bc_workers[1]), 3)
  ...
  @test.assert_eq(ch.worker_for_key("alpha"), key_a)
...
test "README · watch 监视目标，目标终止后收到一条 Terminated 通知" {
  ...
  @test.assert_eq(sys.state_of(watcher), Some(1))
...
test "README · 有界邮箱在满时按背压策略处置并计数" {
  ...
  @test.assert_eq(dn.offer(1), Enqueued)
  @test.assert_eq(dn.offer(2), Enqueued)
  @test.assert_eq(dn.offer(3), Dropped)
  @test.assert_eq(dn.length(), 2)
  @test.assert_eq(dn.dequeue(), Some(1))
  @test.assert_eq(dn.dequeue(), Some(2))
  // counts = (enqueued, dropped, rejected)
  @test.assert_eq(dn.counts(), (2, 1, 0))
  ...
  let _ = dolds.offer(1)
  let _ = dolds.offer(2)
  @test.assert_eq(dolds.offer(3), Dropped)
  @test.assert_eq(dolds.dequeue(), Some(2))
  @test.assert_eq(dolds.dequeue(), Some(3))
  ...
  let _ = rj.offer(1)
  let _ = rj.offer(2)
  @test.assert_eq(rj.offer(3), Rejected)
  @test.assert_eq(rj.length(), 2)
  @test.assert_eq(rj.counts(), (2, 0, 1))
  // 容量上界：始终不超过 capacity
...
test "README · 受监督工作池端到端（无失败与单点失败两种场景）" {
  ...
  @test.assert_eq(clean.completed, 6)
  @test.assert_eq(clean.restarts, 0)
  // coordinator 监视全部 3 个 worker，停止阶段恰各收到一条 Terminated
  @test.assert_eq(clean.terminated_seen.length(), 3)
  // ask 取回各 worker 完成计数：每个完成 2 个
  @test.assert_eq(clean.ask_results, [Replied(2), Replied(2), Replied(2)])

  // ② 单点失败：fault_at = Some(1) → OneForOne 仅重启 worker#1，未失败结果不丢失
  let faulted = worker_pool_demo(3, tasks, Some(1), 7)
  @test.assert_eq(faulted.restarts, 1)
  // 可完成任务数 = 总数 6 − 注入失败 1 = 5
  @test.assert_eq(faulted.completed, 5)
  @test.assert_eq(faulted.terminated_seen.length(), 3)
  // 被重启的 worker#1 重启后仅完成 1 个，其余各 2 个
  @test.assert_eq(faulted.ask_results, [Replied(2), Replied(1), Replied(2)])

  // 确定性可重放：同输入同种子两次运行结果逐字段一致
  @test.assert_eq(worker_pool_demo(3, tasks, Some(1), 7), faulted)
}
```
